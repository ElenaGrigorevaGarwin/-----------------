# модель с Матерновским и периодическим ядрами
import GPy
import numpy as np
import pandas as pd
...
for department in df.columns:
  ...
  # Создание и обучение модели Гауссовского процесса с Матерновским и периодическим ядрами
  kernel_matern = GPy.kern.Matern52(input_dim=1)
  kernel_periodic = GPy.kern.StdPeriodic(input_dim=1)
  kernel = kernel_matern + kernel_periodic  # Комбинация ядер
...
predictions.to_excel("result/Прогнозы_GP_Mater_Per.xlsx")